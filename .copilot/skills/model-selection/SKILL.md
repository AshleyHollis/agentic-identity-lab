...
**Layer 1.5 — Copilot CLI Host Session Model:** No explicit override above? Check the user's host CLI selection by reading (in order):
1. `$env:COPILOT_MODEL` environment variable
2. `$env:COPILOT_HOME/settings.json` if `COPILOT_HOME` is set, looking for the `model` key
3. `~/.copilot/settings.json`, looking for the `model` key

If any returns a model name, treat it as the user's session preference and use it. **Limitation:** Copilot CLI does not expose runtime in-session model picker changes to skills — if the user switches models via the picker mid-session, only the env var and settings.json values are visible. Direct them to `.squad/config.json` `defaultModel` or a chat directive ("use gpt-5.5") if they want their picker change to propagate.
...
|------|--------------|-----|---------------|
| Core Dev / Backend / Frontend | `claude-sonnet-4.6` | Writes code — quality first | Heavy code gen → `gpt-5.3-codex` (LTS codex) |
...
- **Switch to code specialist (`gpt-5.3-codex`):** large multi-file refactors, complex implementation from spec, heavy code generation (500+ lines). `gpt-5.3-codex` is the long-term-support codex line; older codex variants (`gpt-5.2-codex`, `gpt-5.1-codex*`) are deprecated.
...
```
Premium:  claude-opus-4.7 → gpt-5.5 → claude-opus-4.6 → claude-opus-4.6-fast → claude-opus-4.5 → claude-sonnet-4.6 → (omit model param)
Standard: claude-sonnet-4.6 → gpt-5.5 → gpt-5.4 → gpt-5.3-codex → claude-sonnet-4.5 → claude-sonnet-4 → (omit model param)
Fast:     claude-haiku-4.5 → gpt-5.4-mini → gpt-5.4-nano → gpt-4.1 → gpt-5-mini → (omit model param)
```
...
**Standard:** `gpt-5.5`, `claude-sonnet-4.6`, `gpt-5.4`, `gpt-5.3-codex`, `claude-sonnet-4.5`, `claude-sonnet-4`, `gemini-3-pro-preview`, `gemini-3.1-pro`
**Fast/Cheap:** `claude-haiku-4.5`, `gpt-5.4-mini`, `gpt-5.4-nano`, `gpt-5.1-codex-mini`, `gpt-5-mini`, `gpt-4.1`, `gemini-3-flash`

**Deprecated (do not select):** `gpt-5.2`, `gpt-5.2-codex`, `gpt-5.1`, `gpt-5.1-codex`, `gpt-5.1-codex-max`. Falling back to these is harmless (they're still in the catalog for backwards compat) but never select them as Layer 1/2/3 first choice.
...
- Layer 3 base: `claude-sonnet-4.6`
